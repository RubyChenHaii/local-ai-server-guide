...
| 參數規模 | 特性 | 適合場景 |
|----------|------|----------|
| 0.6B – 3B | 迷你模型，邏輯較弱 | 手機、文書筆電、簡單指令、自動化腳本觸發器 |
| 4B – 8B | 主流標準，CP 值最高 | 日常對話、基礎代碼撰寫、輕度資料整理 |
| 9B – 14B | 邏輯能力顯著提升 | 複雜代碼 Debug、資料分析、多步驟推理 |
| 30B – 35B | 具備強大推理能力 | 架構設計、長篇邏輯推演、深度研究輔助 |
| 70B+ | 邏輯接近頂尖商業模型 | 旗艦任務，需高階工作站或多 GPU |

#### 各規模的真實能力與幻覺傾向

這是選模型時最容易被忽略的面向。下表說明不同規模在**事實正確性**上的真實表現——亦即「說瞎話」的機率有多高。

| 參數規模 | 事實幻覺風險 | 指令跟隨 | 代碼能力 | 多步驟推理 | 使用前須知 |
|----------|-------------|----------|----------|------------|-----------|
| 0.6B – 3B | 🔴 非常高 | 尚可 | 弱 | 差 | 極容易一本正經說瞎話。適合格式轉換、分類等結構化任務，不適合需要事實正確的問答。 |
| 4B – 7B | 🟠 偏高 | 良好 | 中等 | 弱至中 | 事實性問答仍有明顯幻覺（實測約 80–84%）。但指令跟隨和簡單代碼能力已相當實用。 |
| 8B – 9B | 🟡 中等 | 良好 | 良好 | 中等 | 開始具備實質解題能力的門檻。日常對話、基礎代碼撰寫體驗接近商業模型。 |
| 14B – 20B | 🟢 偏低 | 優秀 | 優秀 | 良好 | 幻覺風險顯著下降。複雜邏輯推理能力大幅提升，適合代碼 Debug 和多步驟分析。 |
| 30B – 35B | 🟢 低 | 優秀 | 優秀 | 優秀 | 推理品質接近商業模型。在 96GB RAM 上可用較慢速度跑完整推理鏈。 |
| 70B+ | 🟢 很低 | 頂尖 | 頂尖 | 頂尖 | 邏輯與事實準確性接近 GPT-4 等級。需高階硬體支援。 |

> **⚠️ 重要說明：參數量不是唯一指標**
>
> 上表是通用趨勢，但實際表現會因**模型架構、訓練資料品質、後訓練方式**而有很大差異。以 2025–2026 年的現代模型為例：
>
> - **Qwen3-4B**（思考模式）在部分編程基準上可超越舊世代 72B 模型，因為 Qwen3 系列使用了大量強化學習和蒸餾訓練。
> - **幻覺的類型**也很重要：小模型在「事實性知識問答」上幻覺率高，但在「指令跟隨」和「格式化輸出」上表現通常良好。
> - 選模型的原則：**先確認你的主要使用場景是「事實查詢」還是「任務執行」**，再根據硬體選最大能跑的規模。
...
下表以 Q4_K_M 量化為基準，協助你根據手邊的硬體選擇最合適的模型規模。除了速度數字之外，也說明每種配置的**真實使用體驗**與**適合的使用者類型**。

| 硬體等級 | 代表規格 | 建議模型 | 預期速度 | 真實體驗 |
|----------|----------|----------|----------|----------|
| 文書筆電（無獨顯） | 無專用 VRAM / 8–16GB RAM | 1B–3B Q4 或 Q6 | 3–10 t/s | 速度緩慢，回應品質有限。適合做文字格式整理、簡單摘要，不建議用於複雜推理。 |
| Apple Silicon（MacBook） | Unified Memory 8–16GB | 7B Q4（8GB）/ 13B Q4（16GB） | 20–35 t/s | Metal 加速使效率遠優於同規格 Windows 筆電。16GB 版本跑 13B 模型體驗已相當流暢。 |
| 入門筆電（獨顯） | 4–6GB VRAM / 16GB RAM | 7B Q4_K_M | 15–25 t/s | 僅能跑 7B 以下模型，稍大一點就會溢出到 RAM 掉速。適合日常對話，代碼能力尚可。 |
| 中階電競筆電 | 6–8GB VRAM / 16–32GB RAM | 7B Q6 或 8B Q4 | 20–35 t/s | 可完整跑 7B–8B 模型而不溢出，日常使用體驗流暢。是入門本地 AI 的性價比甜蜜點。 |
| 高階電競筆電 / 桌機 | 12GB VRAM / 32GB+ RAM | 9B Q6（速度）或 35B Q3（品質） | 30–50 t/s 或 3–8 t/s | 這個等級是重要分水嶺：9B Q6 可完全住在 VRAM 內跑出流暢速度；若想追求推理品質，可改用 35B，但需忍受 3–8 t/s 的龜速。 |
| 頂級桌機（大 RAM） | 12GB VRAM / 64GB+ RAM | 9B Q6（主力）或 35B–70B Q4（實驗） | 30–50 t/s 或 1–5 t/s | RAM 超大是你的優勢：可以把 70B 模型完整載入記憶體實驗，雖然每秒只出 1–2 個字，但能完成一般人辦不到的深度推理任務。 |
| 高階工作站 | 24GB VRAM | 30B–35B Q4/Q5 | 15–25 t/s | 大模型完全住在 VRAM，推理品質高且速度穩定。適合需要長期、高品質輸出的專業使用者。 |
| 雙 GPU 工作站 | 48GB+ VRAM | 70B Q4/Q6 | 20–40 t/s | 可流暢跑 70B 級別模型，邏輯與創作能力接近商業 API，同時保有完全的隱私與離線能力。 |

#### 如何判斷你適合哪種使用模式？

| 你的主要需求 | 建議策略 |
|-------------|----------|
| 日常對話、寫作輔助 | 選最大能完整住進 VRAM 的模型，流暢 > 品質 |
| 代碼撰寫與 Debug | 8B–14B 是甜蜜點；有 12GB VRAM 的話 9B Q6 是首選 |
| 事實性研究查詢 | 優先選 14B 以上，幻覺率明顯較低 |
| 長篇文件分析 | 加大 Context Length，同時開啟 KV Cache Q4_0 避免溢出 |
| 追求極致推理品質 | 用大 RAM 跑 35B+，接受速度換品質的取捨 |
| 完全隱私、離線使用 | 任何規模皆可，本地 AI 的核心優勢之一 |

> **🖥️ 模型配置範例：96GB RAM + 12GB VRAM**
>
